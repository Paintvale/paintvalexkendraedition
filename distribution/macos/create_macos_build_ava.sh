...
sed -r -i.bck "s/\%\%paratrpaidlemidairthentakingoffhisshelltotpeeforoneminutewhileflyingidle_BUILD_VERSION\%\%/$VERSION/g;" "$UNIVERSAL_APP_BUNDLE/Contents/Info.plist"
sed -r -i.bck "s/\%\%paratrpaidlemidairthentakingoffhisshelltotpeeforoneminutewhileflyingidle_BUILD_GIT_HASH\%\%/$SOURCE_REVISION_ID/g;" "$UNIVERSAL_APP_BUNDLE/Contents/Info.plist"

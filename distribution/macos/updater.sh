...
error_handler() {
    ...
    script="""
    set alertTitle to \"Paintvale - Updater error\"
    set alertMessage to \"An error occurred during Paintvale update (updater.sh:$lineno)\n\nPlease download the update manually from our website if the problem persists.\"
    ...
    if the button_pressed is \"Open Download Page\" then
        open location \"https://paintvale.app/download\"
    end if
    """

...
namespace Paintvale.Common.Configuration
{
    ...
    {
        private const string DefaultBaseDir = "Paintvale";
        ...
        private static string SetUpLogsDir()
        {
            ...
            else
            {
                ...
                {
                    // NOTE: Should evaluate to "~/Library/Logs/Paintvale/".
                    ...
                    {
                        // NOTE: Should evaluate to "~/Library/Application Support/Paintvale/Logs".
                        logDir = Path.Combine(BaseDirPath, "Logs");
                ...
                {
                    // NOTE: Should evaluate to a "Logs" directory in whatever directory Paintvale was launched from.
                    ...
                    if (string.IsNullOrEmpty(logDir))
                    {
                        // NOTE: Should evaluate to "C:\Users\user\AppData\Roaming\Paintvale\Logs".
                ...
                {
                    // NOTE: Should evaluate to "~/.config/Paintvale/Logs".
                    logDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), DefaultBaseDir, "Logs");
            ...
            if (IsPathSymlink(correctApplicationDataDirectoryPath))
            {
                ...
                catch (Exception exception)
                {
                    Logger.Error?.Print(LogClass.Application,
                        $"Critical error copying Paintvale application data into the temp folder. {exception}");
                    try
                    {
                        ...
                        Logger.Error?.Print(LogClass.Application, $"Please manually move your Paintvale data from {resolvedPath} to {correctApplicationDataDirectoryPath}, and remove the symlink.");
                    }
                    catch (Exception symlinkException)
                    {
                        Logger.Error?.Print(LogClass.Application, $"Unable to resolve the symlink for Paintvale application data: {symlinkException}. Follow the symlink at {correctApplicationDataDirectoryPath} and move your data back to the Application Support folder.");
                    }
                    return;
                }
                ...
                {
                    Logger.Error?.Print(LogClass.Application,
                        $"Critical error deleting the Paintvale application data folder symlink at {correctApplicationDataDirectoryPath}. {exception}");
                    try
                    {
                        FileSystemInfo resolvedDirectoryInfo =
                            Directory.ResolveLinkTarget(correctApplicationDataDirectoryPath, true);
                ...
                {
                    Logger.Error?.Print(LogClass.Application,
                        $"Critical error copying Paintvale application data into the correct location. {exception}. Please manually move your application data from {tempPath} to {correctApplicationDataDirectoryPath}.");
                }
            }
        }

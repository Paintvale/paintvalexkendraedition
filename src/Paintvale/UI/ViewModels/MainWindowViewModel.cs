...
using Paintvale.Ava.Input;
using Paintvale.Ava.UI.Controls;
using Paintvale.Ava.UI.Helpers;
using Paintvale.Ava.UI.Models;
using Paintvale.Ava.UI.Models.Generic;
using Paintvale.Ava.UI.Renderer;
...
using Key = Paintvale.Input.Key;
using MissingKeyException = LibHac.Common.Keys.MissingKeyException;
using ShaderCacheLoadingState = Paintvale.Graphics.Gpu.Shader.ShaderCacheState;
    ...
    public partial class MainWindowViewModel : BaseModel
    {
        ...
        public static readonly Bitmap IconBitmap =
            new(Assembly.GetAssembly(typeof(MainWindowViewModel))!
                .GetManifestResourceStream("Paintvale.Assets.UIImages.Logo_Paintvale_AntiAlias.png")!);
...
#pragma warning restore MVVMTK0034
            ...
            if (result.Count > 0)
            {
                ...
                await Dispatcher.UIThread.InvokeAsync(async () =>
                {
                    await ContentDialogHelper.ShowTextDialog(
                        LocaleManager.Instance[numAdded > 0 || numRemoved > 0 ? LocaleKeys.PaintvaleConfirm : LocaleKeys.PaintvaleInfo],
                        msg, 
                        string.Empty, 
                        string.Empty, 
                        string.Empty, 
        ...
        public async Task InstallFirmwareFromFile()
        {
            IReadOnlyList<IStorageFile> result = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
            {
                ...
                FileTypeFilter = new List<FilePickerFileType>
                {
                    ...
                    {
                        Patterns = ["*.xci", "*.zip"],
                        AppleUniformTypeIdentifiers = ["com.paintvale.xci", "public.zip-archive"],
                        MimeTypes = ["application/x-nx-xci", "application/zip"],
                    },
                    new("XCI")
                    {
                        Patterns = ["*.xci"],
                        AppleUniformTypeIdentifiers = ["com.paintvale.xci"],
        ...
        public void OpenPaintvaleFolder()
        {
        ...
        {
            await NavigationDialogHost.Show(AccountManager, ContentManager, VirtualFileSystem, LibHacHorizonManager.PaintvaleClient);
        }
        ...
        public async Task OpenFile()
        {
            IReadOnlyList<IStorageFile> result = await StorageProvider.OpenFilePickerAsync(new FilePickerOpenOptions
            {
                ...
                FileTypeFilter = new List<FilePickerFileType>
                {
                    ...
                    {
                        Patterns = ["*.nsp"],
                        AppleUniformTypeIdentifiers = ["com.paintvale.nsp"],
                    ...
                    {
                        Patterns = ["*.nca"],
                        AppleUniformTypeIdentifiers = ["com.paintvale.nca"],
                        MimeTypes = ["application/x-nx-nca"],
                    },
                    new("NRO")
                    {
                        Patterns = ["*.nro"],
                        AppleUniformTypeIdentifiers = ["com.paintvale.nro"],
                        MimeTypes = ["application/x-nx-nro"],
                    },
                    new("NSO")
                    {
                        Patterns = ["*.nso"],
                        AppleUniformTypeIdentifiers = ["com.paintvale.nso"],
        ...
        public void AppHost_AppExit(object sender, EventArgs e)
        {
            ...
            Dispatcher.UIThread.InvokeAsync(() =>
            {
                Title = PaintvaleApp.FormatTitle();
            });
        }
